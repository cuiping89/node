...
本方案旨在实现一个功能强大、灵活且高度伪装的多协议代理节点。核心思想是通过 单端口复用 和 内部回环 技术，在保证性能的同时，最大限度地提高安全性。

### 2. 端口与协议分配策略

  - **端口分配**：单端口复用 + 内部回环
  - 技术原理：脚本采用单端口复用架构，即 **Xray 监听 443 端口**作为所有流量的唯一入口。它会自行处理 Reality 协议，并将非 Reality 流量（如 gRPC 和 WebSocket）通过 **`fallbacks` 机制转发到 Nginx**。Nginx 仅监听**内部回环端口**（如 `10443`）和**公网 80 端口**（用于提供订阅服务），从而避免了端口冲突。
- 对外开放端口（GCP 防火墙配置）
...
- Nginx Stream (监听 127.0.0.1:10443)
  ...
  - 根据客户端的 ALPN（应用层协议协商） 值进行分流：
    ...
    - 实现细节：优先使用 $ssl_preread_alpn 做 ALPN 分流；若当前发行版不支持该变量，则自动回退为基于 $ssl_preread_server_name 的 SNI 分流（不改变对外协议与订阅字段）。
- 后端服务
